...
def get_args():
    parser = argparse.ArgumentParser(
        description='Alertmanager Webhook Receiver to Send Notifications to Discord, Slack, Telegram and PagerDuty'
    )
...
def validate_config(conf):
    ...
    if 'discord' not in conf and 'telegram' not in conf and 'slack' not in conf:
        raise KeyError('None of "discord", "telegram", or "slack" found in config.yml')
    ...
    if 'slack' in conf and 'bot_token' not in conf['slack']:
        raise KeyError('"bot_token" not found under slack section of config.yml')

    if 'slack' in conf and 'bot_username' not in conf['slack']:
        raise KeyError('"bot_token" not found under slack section of config.yml')
    ...
    if 'slack' in conf and 'environments' not in conf['slack']:
        raise KeyError('"environments" not found under slack section of config.yml')
    ...
    if 'slack' in conf:
        for env in conf['slack']['environments']:
            for severity in conf['slack']['environments'][env]:
                if 'channel_id' not in conf['slack']['environments'][env][severity]:
                    raise KeyError(f'"channel_id" not found for severity {severity} for the {env} environment for Slack in config.yml')

                if 'icon_url' not in conf['slack']['environments'][env][severity]:
                    raise KeyError(f'"icon_url" not found for severity {severity} for the {env} environment for Slack in config.yml')
...
def parse_alert_message(notification_system, title, message):
    ...
    elif notification_system == 'slack':
...
def slack_handler(severity):
    ...
    for alert in payload["alerts"]:
        ...
        if title is None and description is None:
            logging.info("[SLACK]: No title or description, no notification will be sent")
            continue

        # environment label must be present
        if "environment" not in alert["labels"]:
            logging.info("[SLACK]: No environment label, no notification will be sent")
            continue
        ...
        slack_config = config[notification_system]["environments"][environment][severity]
        ...
        blocks = [
            ...
            },
            {
                ...
                "elements": [
                    ...
                    {
                        "type": "mrkdwn",
                        "text": f"*Status:* {alert['status']} | *Hostname:* {hostname} | *Application:* {application}"
                    }
                ]
            }
        ]
        ...
        try:
            ...
            response = client.chat_postMessage(
                ...
                blocks=blocks,
                username=config[notification_system]["bot_username"]
            )
            responses.append(response)
        except Exception as e:
            logging.error(f"[SLACK]: Failed to send message: {e}")
    ...
    return make_response(jsonify(
        {
            ...
            'slack': discord_handler(severity),
